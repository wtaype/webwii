import{j as i}from"./vendor-gzd0YkcT.js";import{db as p}from"./firebase-42xi8Tzz.js";import{s as I,d as y,b as S,c as j,e as x,q as C,f as q,o as z}from"./firebase-XdCz69j8.js";import{g as v,N as o,h as f,j as $,k as w,y as B,f as m,s as H}from"./main-NDLKTLLv.js";import"./main-B0GQYiyC.js";const u="wii_diseno";let l=[],c=[],n=!!v("wiSmile");const D=r=>(r||"").toLowerCase(),k=r=>r.trim().toLowerCase().replace(/\s+/g,""),A=Array(8).fill('<div class="dis_skel"><div class="dis_skel_img shimmer"></div><div class="dis_skel_t shimmer"></div><div class="dis_skel_s shimmer"></div></div>').join(""),U=()=>`
  <div class="dis_wrap">
    <div class="dis_topbar">
      ...
      <button id="dis_refresh" class="dis_refresh_btn" ${m("Actualizar")}><i class="fas fa-rotate-right"></i></button>
    </div>
    ...
    <div class="modalBody dis_modal_body">
      ...
      <div class="dis_card" data-id="${a.id}" style="transition-delay:${Math.min(e*40,400)}ms">
        <div class="dis_card_img">
          ...
          <div class="dis_over">
            <a href="${a.url}" target="_blank" rel="noopener" class="dis_over_btn" ${m("Abrir")}><i class="fas fa-external-link-alt"></i></a>
            ${n?`<button class="dis_over_btn dis_edit" data-id="${a.id}" ${m("Editar")}><i class="fas fa-edit"></i></button><button class="dis_over_btn dis_del" data-id="${a.id}" ${m("Eliminar")}><i class="fas fa-trash"></i></button>`:""}
          </div>
        </div>
      ...
      </div>`))};i("#dis_form").on("submit",async function(d){d.preventDefault();const s=i(this).data("editId"),a=s||`${k(i("#dis_id").val())}_${Date.now()}`,e={nombre:i("#dis_nombre").val().trim(),url:i("#dis_url").val().trim(),img:i("#dis_img").val().trim(),descripcion:i("#dis_desc").val().trim(),tags:i("#dis_tags").val().split(",").map(t=>t.trim()).filter(Boolean),favorito:+i("#dis_fav").val(),autor:v("wiSmile")?.usuario||"admin",fecha:j()};if(!e.nombre||!e.url)return o("Completa los campos","warning");f("#dis_save");try{await I(y(p,"diseno",a),e,{merge:!0}),o(s?"Actualizado ✓":"Guardado ✓","success"),$("modalDiseno"),await r(!0)}catch{o("Error al guardar","error")}finally{f("#dis_save",!1)}}),i(document).on("click",".dis_edit",function(d){d.stopPropagation();const s=l.find(a=>a.id===i(this).data("id"));s&&(i("#dis_modal_tit").html('<i class="fas fa-edit"></i> Editar Herramienta'),i("#dis_nombre").val(s.nombre),i("#dis_id").val(s.id).prop("readonly",!0),i("#dis_url").val(s.url),i("#dis_img").val(s.img),i("#dis_desc").val(s.descripcion),i("#dis_tags").val((s.tags||[]).join(", ")),i("#dis_fav").val(s.favorito||0),i("#dis_form").data("editId",s.id),w("modalDiseno"))}),i(document).on("click",".dis_del",async function(d){if(d.stopPropagation(),!!confirm("¿Eliminar?"))try{await S(y(p,"diseno",i(this).data("id"))),o("Eliminada ✓","success"),await r(!0)}catch{o("Error al eliminar","error")}}),i(document).on("click","#dis_agregar",()=>{i("#dis_form")[0].reset(),i("#dis_form").removeData("editId"),i("#dis_id").prop("readonly",!1),i("#dis_modal_tit").html('<i class="fas fa-palette"></i> Agregar Herramienta'),w("modalDiseno")}),i("#dis_nombre").on("input",function(){i("#dis_id").prop("readonly")||i("#dis_id").val(k(i(this).val()))});let h;i("#dis_search").on("input",function(){i("#dis_search_clr").toggle(!!i(this).val().trim()),clearTimeout(h),h=setTimeout(_,220)}),i("#dis_search_clr").on("click",()=>i("#dis_search").val("").trigger("input").focus()),i("#dis_cancel").on("click",()=>$("modalDiseno")),i("#dis_orden").on("change",_),i("#dis_refresh").on("click",E);let b=0;i(document).on("click",".dis_titulo",()=>{++b>=7&&(n=!0,b=0,g(c),o("¡Dios te Ama! 🙏","success"))}),r(),console.log(`🎨 Diseño · ${B()}`)},G=()=>{n=!!v("wiSmile"),console.log("🧹 Diseño")};export{G as cleanup,F as init,U as render};
